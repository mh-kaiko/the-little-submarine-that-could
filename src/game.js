...
(() => {
  ...
  function placeFeature(th) {
    ...
    // Explicit lower bound: a rock roll that found no room must not fall through into a crate.
    if (th.box && th.crate > 0 && r >= th.vent + th.rock && r < th.vent + th.rock + th.crate && gap >= TPASS + 60) {
  ...
  function update(dt) {
    ...
    if (!zone.boss) {
      G.spawnT -= wdt;
      if (G.spawnT <= 0) {
        const interval = lerp(2.4, 0.9, difficulty) * (zone.spawnMul || 1);
        G.spawnT = interval * rand(0.7, 1.3);
        spawnEnemy(weightedPick(zone.enemies.map(([k, w]) => ({ w, v: k }))));
        if (difficulty > 0.35 && Math.random() < (zone.pair ?? 0.3)) spawnEnemy(weightedPick(zone.enemies.map(([k, w]) => ({ w, v: k }))));
      }
      G.hazardT -= wdt;
